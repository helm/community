...
The true value of leveraging OCI specs has very little to do with the command-line experience (i.e. Docker). OCI registries provide a common API for all types of packages, and address several security and scalbility concerns. Additionaly, many companies and organizations have invested in the infrastructure surrounding their container registry. Storing Helm charts in a registry reduces the number of moving parts.
...
Since the existing feature set is currently experimental, there will be no promise of backwards compability with prior OCI support (sorry!)
...
Additionaly, site containing community charts, such as [Artifact Hub](https://artifacthub.io/), should enable providers to distribute charts over OCI, and display friendly copy-paste instructions for how to download an OCI-based chart.
...
Again, there doesn't appear to be an IANA media type for this type of file. Additionaly, this file format appears unique to Helm.
...
Its unclear if users will be able to pull charts with a specific version using the `:<tag>` syntax (vs. `--version <version>`). This seems relatively harmless to allow both, but this may have implications on depdencies and others areas.

## References

The following links are to used as references in this HIP:
