...
A third approach would be trying to `grep` the images references out of the Chart files. Again, this ends up being an attempt of promoting self-made conventions in order to make the _grepping_ work. Discussing this point, we brought Operators to the table. These components may rely on images which aren't declared on the helm files, but pulled directly from the source code.
