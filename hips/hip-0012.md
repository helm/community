...
With KubeCon/CloudNativeCon North America 2024 marking the fifth anniversary of the successful Helm 3 release, the development of Helm 4 is strongly desired in order to continue evolving Helm and its ecosystem.
...
This HIP also aims for an open and productive process that enables Helm 4 to build upon the success of Helm 3 and continue evolving Helm for the benefit of the Helm community.

## Motivation

Helm 3 has been greatly successful, and Helm has built a significant ecosystem around the Helm client (CLI), charts, chart repositories, SDK, plugins, and so on.
...
One significant issue with Helm 3 is that making many good changes is overly difficult due to Helm's [HIP-0004](HIP-0004) backwards compatibility guarantees –- behavioral or API incompatible changes are not allowed during regular minor/patch version releases.
...
- Requiring changes to be documented, either as “Helm 4 proposals” or standard HIPs, allows both Helm community members and maintainers to review them.
...
For changes that don't make it into Helm 4, there will be a Helm 5 release of course!
...
Helm v4.1.0 will be in January 2026 and from then on it will follow the same release cadence that Helm v3 has, shadowing Kubernetes by approximately 1 month.

Helm 3 will reach end of life six months after Helm 4 release (July. 2026 estimated)

### Change suitability

The Helm ecosystem is large, encompassing not just the Helm CLI but also the SDK, Charts, and Chart repositories, plugins, and so on.
...
These proposals can take the form of lighter-weight Helm 4-specific proposals or standard HIPs suitable for Helm 4 implementation.
Once a Helm 4 proposal is deemed suitable, it will be marked as implementable in the `hips/helm-v4` directory under `hips/helm`.
Standard Helm HIPs that are marked suitable for Helm 4 will also be accepted.

#### Helm 4 proposals
A "Helm 4 proposal" is a lighter-weight alternative process to a standard Helm HIP.
Designed to minimize the friction required for implementing small, functional changes in Helm 4 that do not warrant a full HIP process.
And specifically intended to avoid development of features from e.g. Github issues.
A "Helm 4 proposal" will follow the same format as a HIP, but the author can choose to skip sections they do not feel applicable.
Helm maintainers may choose to promote a "Helm 4 proposal" to be a standard HIP should they feel the change is significant enough.

### Compatibility

Even though Helm 4 is a major version release not bound by [HIP-0004](HIP-0004) compatibility guarantees, it must remain largely feature/functionality compatible with Helm 3.
Due to Helm's maturity and the extensive ecosystem built around its CLI, charts, SDK, and so on.
...
Similarly, any release manageable by Helm 3 must be upgradable by Helm 4.
...
The engineer should not be at risk of losing their core maintainer status due to inactivity.
Self-nominations are encouraged, and potential nominees must agree to the nomination.
...
The approval process for PRs remains standard, but any PR not aligned with the current plan must receive approval from the Release Engineer.
...
Specifically, [HIP-0004](HIP-0004) pertains only to minor and patch releases, not major version releases.
This does not affect the PR review process or the HIP process, as both are expected to remain unchanged.

## How to teach this

The plans for Helm 4 are scheduled to be announced during KubeCon/CloudNativeCon North America 2024 at both the Helm Maintainers talk and Helm 'contribfest'.
...
Furthermore, there is a paucity of documentation detailing past decisions made in Helm 2, with only a few draft documents available in Helm 3.

Therefore, HIPs and their lighter-weigtht couterparts "Helm 4 proposals" will serve as a platform for public discourse and provide a recorded decision-making process.
