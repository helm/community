...
<!-- process -->
Each significant change or feature needs to have a sponser who will work with the release manager and be accompoinent by a design proposal. Similar to Helm 3 proposals: [helm/community/archieves/helm/helm-v3](archieves/helm/helm-v3), or taken from standard HIPs. Before begining implementation, architectural oversight from senior maintainers.

In order to produce Helm 4 in a timely fashion with limited resources, it is anticipated that timeboxing the Helm 4 development lifecycle will be needed. It will be kicked off in X, 2024. The process will last not less than six months, and not more than 1 year, and changes managed by a release engineer.
...
- Given the limited amount of community resource to enact changes, it is considered much more important to get a Helm 4 release which can deliever significant benefits. Than a prolonged process which takes to long to deliver meaningful results.
- With the significant ecosystem built around the Helm: the client, Chart's, the SDK, etc, Helm 4 changes _must_ be largely functionally/feature compatible with Helm 3 in order to maintain coherency in the Helm ecosystem. Breakage of signifcant features/functionmality used by the community without sufficent migration options will at best delay Helm 4 update and incur cost to Helm's users. Or at worst splinter the Helm ecosystem.
...
By requiring changes to be documented (Helm4 proposals, or standard HIPs), Helm maintainers can provide architectual oversight to the changes. Ensuring the are in good standing with respect to this HIP. Then allowing the contributors to independantly execute changes to the agreed proposal.
...
- behavioural changes that negatively affect a significant number of users can't be allowed
...
In order to ensure Helm 4 changes are aligned to this HIP, Helm mantainers will review proposals. Either in the form of Helm 4 specific proposals (intended to be a lighter-weight process to HIPs). Or HIPs, which deemed to be suitable for Helm 4 implementation.

When a Helm 4 proposal is deemed suitable, it will be marked as implementable in the hips/helm-v4 directory.
