...
The [high-level goal](https://helm.sh/docs/topics/plugins/#an-overview) of the Helm 3 plugin system shows that we did intend for plugins to solve this problem:
...
- Helm 3 plugins call a user-specified arbitrary binary through a subprocess, which does not allow Helm to limit the scope of these commands (see Wasm sandboxing under "Security implications" below for how this will be resolved). Plugin maintainers must also be concerned about OS and architecture compatbility of pre-built binaries and scripts.
...
We will implement a Wasm plugin architecture in Helm 4 that componentizes core Helm functionality into discrete, replaceable plugins. The ultimate goal is to move as much existing functionality/code as possible out of Helm core into default plugins.
...
Like all other Helm 4 plugins, these will be written as Wasm plugins. However, while non-default plugins may be written in any language supported by Extism PDK, Default Behavior plugins will be written in Go (effective copied from existing Helm code), in order to keep Helm source codebase—core and default plugins—consistent and more maintainable than a polyglot codebase.
...
- **Postrender** (the [Helm 3 post-renderer](https://helm.sh/docs/topics/advanced/#post-rendering) functionality moved to the new Wasm plugin system. See Security implications section)
...
> [George: move most of this to rejected ideas]While Helm 3 currently allows additional keys to be added to the Chart struct while retaining backwards-compatibility, any keys that add required functionality for a chart to install would not be forwards compatible with older versions of Helm (such as renderer plugins). Therefore—for practical reasons—we will wait for Chart API v3 before adding a top level `plugins` key to `Chart.yaml`. `Chart.lock` will also reflect this.
...
After the initial plugin system release, we intend to make it easy to continue adding new chart-defined plugin types to extend additional categories of non-default chart behavior as this becomes desirable. Some examples may be:
...
To plan for forward compatibility, a `minimumHelmVersion` field may be added to allow future plugins to specify the minimum version of Helm that must be used for the chart (since this is the version of Helm that introduced the new plugin, perhaps this can be auto-detected). 

Note that Helm 3 allowed a similar forwards-compatibility for Chart API v2, for example when support for a new gotemplate/sprig function was added to a specific Helm version. Charts could check the Helm version in their template or a helper function, and exit with a friendly error if the version of Helm did not meet this version constraint. We may be able to do better as this applies to plugins in Helm 4.
...
plugins:
    render:
        - https://github.com/pkl-community/helm-pkl@0.1.0: [".pkl", ".pickle"]
        - https://github.com/kubeys/helmys@0.1.4: [".ys"]
    download:
...
The new plugin system will continue to allow users to build and install CLI plugins, which specify subcommands for the Helm CLI. However, these will also be run in Wasm sandbox rather than directly calling arbitrary executables as a subprocess as was implemented in Helm 3. This will require Helm 4 plugin developers to follow documentation which will be created detailing the process for migrating legacy CLI plugins to the new Wasm CLI plugin framework.
...
- `source` is added to encourage best security practices. See [Security implications](#security-implications).
- `type` is added to specify the type of plugin.
- `apiVersion` is added to specify the version of the API of the plugin type.

```yaml
name: my-custom-render-tool
version: 0.1.0
apiVersion: plugins.helm.sh/render/v1 # Message scheme the plugin expects to handle
type: render # see helm plugin types
source: https://github.com/example/my-plugin # source code url
[pluginVersion: v1] # Version of plugin API?? [George]
```
...
Helm users will need to get access to plugin Wasm binaries. When running `helm plugin install` and when a chart requires plugins.

THIS SECTION IN PROGRESS. See open questions

<!-- To maintain control of the ecosystem initially, Helm will only accept plugins signed by Helm maintainers (Helm binary will be built with a set list of acceptable signing keys). This will allow plugins to evolve over time (including incompatible changes), without breaking users by Helm 

- Phase 1: Helm will extend... WIP
...
For Wasm Plugin Interfacing, we will be leveraging the Extism project. See <https://extism.org/> and specifically the Extism [Go SDK](https://github.com/extism/go-sdk) and [Go PDK](https://github.com/extism/go-pdk).
...
- Chart authors are encouraged to follow best security practices (Helm may surface when these are not followed), and users are encouraged to take advantage of these
...
- Opt-in flag:
    ...
    The user will need to pass a flag to explicitly acknowledge and opt-in to downloading if not present, and running these third-party chart plugins.
...
- Adds source URL:
    ...
    Helm 4 plugins `plugin.yaml` also adds a `source` key.

    This is to encourage plugin authors to list their plugin source code URL. 
    
- Security best practices encouraged (and may be surfaced):
    
    Plugin authors are (strongly encouraged or required?) to sign the plugins, <!--and are also strongly encouraged to--> clearly list their signing key(s) in the source code README, and give clear Wasm build instructions for their source code.

    For plugins that follow the above recommendations, end users are strongly encouraged to verify the provenance signature. Users may also inspect the source code before using plugins, and if they wish may build and compare the exported plugin for additional verification.
    
<!--     (Plugins installed from the local filesystem (from source) are excepted from signing requirements, for local dev purposes) -->

<!--
--- 
initial plugins requirements:
- default built into Helm binary (SDK ?)
- non-default (alternative) plugins PGP signed by Helm authors
  - plugins hosted on helm.sh?
initial goals:
- limit problems blast radius by maintain control of plugins
- allow non-backwards compartible changes to initial plugin design


3rd party plugin requirements:
- distribution with provence (ie. plugins are signed)
    - scalable signing mechanism (ie. cosign)
    - scalable distribution e.g. OCI + artifact hub
- tooling to inspect plugins ie. discover source URL, plugin signers, key ids, etc
- require chart operators to accept author's signing key ie. `helm plugin accept-author --key-id ...`

- --unsafe flag that lets operators skip plugin validation
-->
...
1. We will not continue to support the Helm 3 subprocess plugin model in Helm 4, which was used for both CLI plugins and downloader plugins.
...
1. We will not continue to only support gotemplate for rendering manifests as Helm 3 did.

    By allowing alternative rendering engines we accept the additional complexity for users to troubleshoot or contribute to charts that may require learning new rendering engines other than gotemplate. This is a trade-off that we are willing to make in order to satisfy the many requests from the community for this feature.
...
1. We will not create a central repository for plugins.

    This is a non-starter for the Helm project. We have learned from the experience of the formerly centralized charts repo that it does not scale well or enable eccosystem growth as much as a distributed community-managed model. See <https://github.com/helm/community/blob/main/hips/archives/helm/distributed-search.md>.

1. We have ruled out the following technologies for plugin interfacing in Helm 4:
    ...
    - [Wadge](https://wasmcloud.com/docs/developer/languages/go/components#wadge) in wasmCloud
        ...
        Problem: This is another CGo-based solution. We would like to avoid CGo due to complexity it would add for cross-compiling, debugging, and other issues.
    ...
    - [Go native plugin system](https://pkg.go.dev/plugin)
        ...
        Problem: This does not support Windows. For this reason alone, we can't use it.

    - [Hashcorp's go-plugin package](https://github.com/hashicorp/go-plugin)
    
        Problem: DESCRIBE THE PROBLEM
        
        Note:This is also what protobuf does (subprocess)

    - [Embedded go interpreter](https://github.com/traefik/yaegi)

        Problem: DESCRIBE THE PROBLEM

        Note: Traefik does this with [yaegi](https://github.com/traefik/yaegi)

    - [Caddy novel Go plugin approach](https://caddyserver.com/docs/architecture)
...
- How strong is it reasonable for Helm to encourage plugin authors to follow security best practices? See Security implications. Strongly encouraged or Required for each of the below:
...
- Should Helm take a phased approach to allowing third-party plugins during the development of this feature and before a secure distribution mechanism is fully rolled out?
    - For example, to start should we only accept plugins signed by Helm maintainers?
- How will we version plugins?
    - Suggested in [Helm plugin specification](#Helm-plugin-specification)
- What are the criteria for which Helm core functionality will be moved to default plugins?
    - The initial implementation of the plugin system should be flexible enough so that we can move additional functionality to plugins even after the initial release of Helm 4, as we hear a strong need from the community for modifying other default behaviors of Helm.
...
- Chart-defined plugins will be required to use the chart. How will these plugins be made available to the end user?
