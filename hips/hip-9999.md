...
The primary place to tecah this feature is in the helm documentation and in the `helm --help` output.
...
I don't know enough about how Helmfile works but if it calls `helm` based on the specification in the file it should work without modification.
Native version locking would still prevent upgrades so no changes should be needed to Helmfiles.
Any other changes to helmfiles was out of scope for this feature.
