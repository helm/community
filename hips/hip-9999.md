...
- **Default plugins**: these will be default plugins for both Chart API v2 and Chart API v3 charts. These should be bundled with Helm 4 so that v2 charts still function exactly like they did in Helm 3, and the new defaults for v3 charts will just work without requiring users to download additional plugins.
...
- improve the security model of existing Helm 3 plugin architecture

### Requirements
- preserve Helm 3 functional backwards-compatibility for Chart API v2 using default plugins, while allowing advanced users to alter charts v2 charts default functionality with alternative plugins
...
- Helm 3 plugins shells out to a user-specified arbitrary binary, which does not allow Helm to limit the scope of these commands (see Wasm sandboxing under "Security implications" below for how this will be resolved).
...
- Plugins will run in a sandboxed environment to mitigate security risks.
    - Unlike when Helm plugins were first introduced, Wasm is now available for sandboxing. It also allows us to clearly define scope a plugin of each type can act upon.
...
- Ideas on the above for **discussion**:
    ...
    - Artifact Hub's verified and official labels could be used to communicate the status of default plugins.
