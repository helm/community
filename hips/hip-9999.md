...
1. Apr. 2025: Engineering on this feature begins
...
- [ ] Create message schemas for new plugins types:
    - [ ] `plugins.helm.sh/download/v1`
    - [ ] `plugins.helm.sh/render/v1`
    - [ ] `plugins.helm.sh/postrender/v1`
    - [ ] `plugins.helm.sh/cli/v1`
- [ ] Write host functions to invoke these initial defined Wasm plugin types
- [ ] Move existing code from Helm core to these initial plugin types for Helm 3 backwards compatibility:
    - [ ] `download`: OCI, HTTP/S
    - [ ] `render`: Gotemplate
...
- [ ] Embed default plugins as part of the default installation of Helm 4

### Chart-defined plugins functionality for Charts v3
...
- [ ] Update existing `pkg/provenance` to:
    - [ ] apply add signing and verification to plugins as well as charts
    - [ ] add signing detection and output (per [plugin commands](#Helm-plugin-commands) listed above)
...
- Create examples of each kind of plugin (Chart-defined plugin, Alternative Behavior plugin, and CLI plugin) for the new plugin system that contributors can use as a model for their own plugins
    - Chart-defined plugin example. The Gotemplate `render` plugin will have already created
    - Default Behavior plugin example. The OCI and HTTP/S `download` plugins will have already created
    - Alternative Behavior plugin example: Create a very simple example. Eg, could write a PR to migrate a community downloader plugin to the new plugin system
    - CLI plugin: Create a very simple example
